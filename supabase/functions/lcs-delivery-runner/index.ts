...
async function sendMailgun(
  ...
  const url = `https://api.mailgun.net/v3/${senderDomain}/messages`;
  ...
  try {
    await client.queryObject(`
      INSERT INTO lcs.event (
        ...
        $1, $2, $3::uuid, $4, $5::uuid,
        $6, $7, $8, $9, $10,
        $11, $12, $13, $14, 6, 'Call Adapter',
        $15::jsonb, $16::jsonb, null, $17
      )
    `, [
      ...
      delivery.entity_id ?? '00000000-0000-0000-0000-000000000000',
      ...
      JSON.stringify(result.adapter_response),
  ...
  try {
    await client.queryObject(`
      INSERT INTO lcs.err0 (
        ...
        $1, $2, $3, $4, $5, $6, $7, 1, 'AUTO_RETRY', false, null
      )
    `, [
      delivery.message_run_id,
      delivery.communication_id,
...
Deno.serve(async (req: Request) => {
  ...
  const pool = new Pool({
    ...
    database: connUrl.pathname.slice(1), // remove leading /
    tls: { enabled: true, enforce: false },
  }, 1); // single connection — edge function is short-lived

  try {
    // ─── Dedup query: QUEUED rows not yet in CET ────
    const client = await pool.connect();
    let deliveries: QueuedDelivery[];
    try {
      const result = await client.queryObject<QueuedDelivery>(`
        SELECT
          mss.message_run_id,
    ...
    if (deliveries.length === 0) {
      await pool.end();
      return new Response(
        JSON.stringify({ status: 'ok', processed: 0, succeeded: 0, failed: 0 }),
    ...
    for (const delivery of deliveries) {
      try {
        ...
        try {
          await insertCetEvent(pool, delivery, {
            event_type: 'DELIVERY_FAILED',
            delivery_status: 'FAILED',
            adapter_response: { error: err instanceof Error ? err.message : String(err) },
            payload: null,

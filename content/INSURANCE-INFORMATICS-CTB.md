...
## The full architecture of how insurance informatics works. Every piece of content slots into an altitude. This is the backbone for the Insurance Informatics FCE (US → K=C → DMJ → UP).
### Status: BUILD
### Medium: doctrine (business backbone)
...
_This doc is the canonical **Insurance Informatics FCE backbone**. It uses the CTB altitude structure (50K → 40K → 30K → 20K → 10K → 5K) in place of UT §4 IMO body — the altitude pattern is CTB native, and the §4 IMO "Middle" for this doc IS that altitude walk. All other UT sections apply._

| # | Check | Status | Location |
|---|-------|--------|----------|
| 1 | PRD — what / why / who / scope / out-of-scope / success metric | ☑ | §0 below + discipline statement at 50K |
| 2 | OSAM — READ / WRITE / Process Composition / Join Chain / Forbidden / Routing | ☐ | CTB body is the content; formal OSAM retrofit pending |
| 3 | Component Status — every dep 🟢 / 🟡 / 🔴 + HEIR + ORBT | ☐ | fixed-side + variable-side vendor list pending status grid |
| 4 | Owner | ☑ | Dave Barton |
| 5 | Live Dashboard | ☑ | Mission Control — Insurance Informatics map |
| 6 | Kill Switch | ☐ | business doctrine — no runtime kill switch applicable |
| 7 | Logbook | ☐ | BUILD phase |
| 8 | FCEs Attached | ☑ | IS the backbone for Insurance Informatics FCE |
| 9 | BARs Referenced | ☑ | BAR-302 (website map), BAR-333 (video pipeline), BAR-166 (pending) |
| 10 | LBB Subjects Fed | ☑ | svg-outreach, svg-sales, svg-client |
| 11 | Geometry — CTB position + Hub-Spoke + Altitude | ☑ | CTB altitude is §4; Geometry is Dave-as-hub, vendors as spokes, client as rim |
| 12 | Live Verification | ☐ | vendor list + current 10/85 team + live rates — pending |
...
### WHAT
The full architecture of Insurance Informatics as a named discipline — how Dave's operation actually works, mapped by altitude. The source-of-truth for every downstream artifact: explainer videos (E-01 to E-05, Deconstructing the Duck), emails, landing pages, sales calls, vendor briefings.

### WHY
Without this doc, every piece of content is invented in isolation and drifts. With it, every artifact — video, email, landing page, LinkedIn post — traces to a specific altitude on the backbone. Structure is the constant. Content is the variable.

### WHO
- **Dave** — operates from the 50K discipline statement; uses lower altitudes to explain
- **Claude / Codex / Sonnet** — read this as the vocabulary source before producing any Insurance Informatics content
- **Sales + service** — reference altitude when talking to prospects/clients
- **Every downstream adapter** (LCS email, video-pipeline, HeyGen, InVideo, content-pages) — pulls phrasings and structure from here

### SCOPE (in)
- The 50K discipline definition + positioning
- 40K two-sides split (Enrollment + Fixed + Variable)
- 30K fixed-side vendor aggregation (10+ PEPM vendors)
- 30K variable-side claim pipes (TPA + PBM)
- 20K 90/15 autopilot + 10/85 high-dollar claimant team
- 10K triggers + flow + waterfall + service model
- 5K specific vendors, specific phrasings, specific CTAs

### OUT-OF-SCOPE
- Voice / tone guidance → `law/VOICE-LIBRARY.md`
- Specific video scripts → `factory/content/video-scripts/EXPLAINER-VIDEOS-E01-E05.md` + `fleet/content/videos/rendered/*.md`
- Worker/code implementation of any adapter → respective worker MANUAL.md
- Engine doctrine → `law/doctrine/FOUNDATIONAL_BEDROCK.md` + `factory/agents/up-v3/FCE_LIFECYCLE.md`

### SUCCESS METRIC
Every Insurance Informatics content artifact produced after this doc locks traces explicitly to an altitude here. Zero "where did this phrasing come from?" questions.
...
### Client-Facing Videos (1 + 4 gates) — ALL EVERGREEN (2026-04-20 pivot)

**Doctrine change 2026-04-20:** personalized per-prospect video rendering RETIRED. Every client-facing video is evergreen, produced once via HeyGen, reused across all prospects. **Personalization happens on the prospect's CF Page** (`docs/processes/CLIENT-CF-PAGE-RUNBOOK.md`), not in the video. Prospect lands on `/c/{slug}?t={token}` → page renders with their data + embeds the appropriate evergreen video.

Rationale: quality + ROI over personalization-at-scale. One HeyGen premium video per altitude is reused for thousands of prospects; personalization moves to the cheap/instant/live layer (HTML + D1 data).

| # | Video | Altitude | Duration | Script Status | Source | Embedded on |
|---|-------|----------|----------|--------------|--------|-------------|
| C-00 | 50K Intro/Filter — what is insurance informatics, why me, "you need me, I don't need you" | 50K | ~2 min | DONE | `hook-the-10-percent-problem.md` | Touch 1 CF Page variant |
| C-01 | Gate 1 — Factfinder ("We Know You") — **evergreen** | 50K/40K | 5-6 min | DONE | `gate-1-source.md` + `gate-1-we-know-you.md` | Gate 1 CF Page variant (post-book) |
| C-02 | Gate 2 — Education + Monte Carlo ("Your Numbers") — **evergreen** | 30K/20K | 5-6 min | DONE | `gate-2-source.md` + `gate-2-your-numbers.md` | Gate 2 CF Page (post Gate 1) |
| C-03 | Gate 3 — Service ("What to Expect") — **evergreen** | 10K/5K | 5-6 min | DONE | `gate-3-source.md` | Gate 3 CF Page |
| C-04 | Gate 4 — Quote + Dare ("What We Found") — **evergreen** | 5K | 3-5 min | DONE | `gate-4-source.md` | Gate 4 CF Page |

**Premium personalized HeyGen video (rare):** when a prospect books Gate 1 AND has ≥3 signal triggers, Dave can approve a single unique HeyGen video with avatar-spoken message for THAT prospect's Gate 1 CF Page variant. Capped at 20-30/month, manual approval, $5-20 per video. This is the moat-moment — reserved for highest intent only, not scale.

**Retired:** BAR-239 personalized gate video rendering per prospect. BAR-333 video-pipeline tool pivots from "personalization engine" to "evergreen video production tool" (HeyGen primary, Remotion optional/retired).
...
| # | Video | Altitude | Duration | Script Status | Source |
|---|-------|----------|----------|--------------|--------|
| E-01 | Self-Insured vs Fully-Insured — "you can't stop claims" | 50K/40K | 1-2 min | ✅ DONE (READY FOR INVIDEO) | `factory/content/video-scripts/EXPLAINER-VIDEOS-E01-E05.md` |
| E-02 | The 90/15 and 10/85 — where the money goes | 30K | 1-2 min | ✅ DONE (READY FOR INVIDEO) | `factory/content/video-scripts/EXPLAINER-VIDEOS-E01-E05.md` |
| E-03 | Hospital Bill Audit — hospitals overbill, here's how we catch it | 20K | 1-2 min | ✅ DONE (READY FOR INVIDEO) | `factory/content/video-scripts/EXPLAINER-VIDEOS-E01-E05.md` |
| E-04 | What Your Broker Can't See — the vendor silo problem | 30K | 1-2 min | ✅ DONE (READY FOR INVIDEO) | `factory/content/video-scripts/EXPLAINER-VIDEOS-E01-E05.md` |
| E-05 | The Duck — smooth on top, paddling like hell underneath | 50K | 1-2 min | ✅ DONE (READY FOR INVIDEO) | `factory/content/video-scripts/EXPLAINER-VIDEOS-E01-E05.md` |
| E-06 (new) | Deconstructing the Duck — full architecture walkthrough | 50K→5K | ~6 min | ✅ DONE (rendered video in hand, transcript saved, pending CF Stream upload) | `fleet/content/videos/rendered/deconstructing-the-duck.md` |
